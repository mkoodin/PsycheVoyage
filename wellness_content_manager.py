...
class WellnessContentManager:
    ...
    def get_previous_content(self, channel_id: str, limit: int = 100) -> List[str]:
        """
        ...
        Returns:
            List of previous content posts
        """
        previous_content = []

        with SessionLocal() as db:
            # First, check the wellness_content table
            stmt = (
            ...
            for item in results:
                if item.content:
                    previous_content.append(item.content)

            # If we don't have enough content, also check other content
            if len(previous_content) < limit:
                try:
                    # Try to get content directly from the wellness_content table
                    # without channel_id filter
                    stmt = (
                        select(WellnessContent)
                        .order_by(desc(WellnessContent.created_at))
                    ...
                    # Extract content from the results
                    for item in results:
                        if item.content and item.content not in previous_content:
                            previous_content.append(item.content)
                except Exception as e:
    ...
    def determine_content_type(self, previous_content: List[str]) -> ContentType:
        """
        Determine the next content type based on rotation and previous content.

        Args:
            previous_content: List of previous content posts
        ...
        # Try to identify the content type of the most recent post
        recent_types = []
        for content in previous_content[:5]:  # Check the 5 most recent posts
            for content_type in ContentType:
                if content_type.value.lower() in content.lower():
                    recent_types.append(content_type)
                    break

        # If we found recent types, choose the next one in rotation
        if recent_types:
        ...
        # Default to meditation tip if we couldn't determine the last type
        logger.info("Could not determine previous content type, using default")
        return ContentType.MEDITATION_TIP
    ...
    def prepare_context(
        ...
        previous_content = self.get_previous_content(channel_id)
        ...
        if content_type and content_type in [ct.value for ct in ContentType]:
            ...
            selected_content_type = self.determine_content_type(previous_content)
